...
6. **Screens are shared MVI, modeled on NYTimes-KMP** (minus its Decompose routing). Each screen is a folder, `screens/<name>/`, with four files. The first three live in `:cupboard`, the fourth in `:cupboard:ui`:
   ...
   Events carry intent-sized facts, one per completed user action. Continuous gestures (drag, resize) stream transient preview events through the loop and commit one event on release; the presenter stashes pre-gesture state, so one gesture is still one undo entry and one autosave write, and a cancel event rolls it back (see `PreviewSlide`/`UpdateSlide`/`CancelPreview` in the editor).

   Two rules the drag-freeze bug taught us (bisected to `9bc97ce`, resolved note in ROADMAP.md):
   - **Anything a gesture shows must come back through `states`.** Never render from composable-local snapshot state written in pointer handlers: on desktop those writes intermittently never reach the recomposer, and the freeze only surfaces on some machines. Non-rendering gesture bookkeeping (drag target, accumulated deltas) lives in plain vars inside the handler and emits absolute values, so a slow roundtrip can delay a repaint but never lose movement.
   - **If the loop looks broken, root-cause it before routing around it.** Two workarounds (serializing the dispatcher, canvas write-behind) each masked the symptom, cost the single source of truth, and had to be reverted once the real defect was traced a layer below.
