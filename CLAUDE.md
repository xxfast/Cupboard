...
6. **Screens are shared MVI, modeled on NYTimes-KMP** (minus its Decompose routing). Each screen is a folder, `screens/<name>/`, with four files. The first three live in `:cupboard`, the fourth in `:cupboard:ui`:
   ...
   Events carry intent-sized facts, one per completed user action. Continuous gestures (drag, resize) stream transient `PreviewSlide` events through the loop (the canvas renders only from roundtripped state; local snapshot writes from pointer handlers proved unreliable for repaint on desktop) and commit one `UpdateSlide` on release, so one gesture is still one undo entry and one autosave write.
