...
- `macosApp/` – SwiftUI host + `CupboardCanvas.framework` (`macosArm64`), built via `Cupboard.xcodeproj`. `Cupboard/HostApp.swift` is the entry + `EditorView` shell; panels are `extension EditorView` files (`Navigator.swift`, `Toolbar.swift`, `Inspector.swift`), with `Theme.swift`, `Window.swift`, `Menus.swift`, `ChromeState.swift` alongside. Sources are a synchronized folder group, so new files need no pbxproj edit.
...
2. **Slide content is document-owned.** Slides render from a serializable document model (slides, elements, builds), not compiled-in composables. A slide's look comes from its deck's theme and ignores the app theme (dark by default, light built-ins exist); fixed 944x531 @1x with container scaling.
