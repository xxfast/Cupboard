...
- **Navigator (left, 224px fixed):** vertically scrolling slide thumbnails. Each row: 16px leading gutter (chevron centred, mono 11px slide number above it) + 136px 16:9 thumbnail, −12px per nesting level, 4–6px gaps; 6px gap between rows; 10px padding (14px at the bottom). Selected thumbnail gets a 2px accent ring. Thumbnails render miniatures of slide content (document-dark gradient `linear-gradient(140deg, #2a2452, #1a1c2e)`), corner radius follows the OS token `thumbR` (mac 5px / win 4px / linux 10px).
...
- **Slide selection:** clicking a navigator thumbnail selects it (accent ring moves, status bar count updates). The mock deck is 10 slides over three levels (a group with a two-level-deep subgroup, plus a second nested pair); slide #4 is selected. See **Slide nesting** for the header/collapse behaviour.
...
The only sizing input is `width` (px); the component derives its own height (16:9) and internal scale, so a consumer just asks for the width it has room for. The canvas passes `width = zoom% × 1920`; the navigator passes 150 (macOS) or 136 (Windows/Linux), minus 12px per nesting level. Editing chrome — selection border, resize handles, guides, build-order badges — is gated behind `interactive`, so thumbnails render the artwork alone.
...
**Disclosure chevron — leading gutter on every platform.** Each row opens with a fixed gutter column (macOS 15px, Windows/Linux 16px) at its leading edge, outside the thumbnail. The chevron sits **vertically centred** in that gutter against the middle of the thumbnail; the slide number sits in the same gutter, tucked to its bottom (macOS) or top (Windows/Linux). Rows without children leave the chevron space empty, so every thumbnail in the list stays left-aligned.

The chevron itself is a **stroked 9×9 glyph** — 2px stroke, round caps and joins, drawn in the `icon` token (not the dimmer `dim`) so it reads as a control rather than a decoration — inside a 16×20 hover target that fills with `hov2`. It points **right when collapsed** and rotates **90° down when expanded**, animating over 140ms ease. In Compose, one chevron vector with an animated `rotationZ`.

**Depth is carried by two cues, not one.** Each nesting level (a) indents the row 12px and (b) steps the thumbnail down 12px in width — macOS 150 / 138 / 126, Windows/Linux 136 / 124 / 112, clamped after three levels. The size step makes depth legible at a glance and guarantees the indent never pushes a thumbnail past the panel's inner edge. The sample deck exercises three levels (10 slides: two nested groups, one of them two levels deep).

**macOS row chrome** matches Keynote on macOS 26 (the Liquid Glass build): the slide number sits *outside* the thumbnail in the leading gutter, bottom-aligned to the thumbnail's lower edge — 11px system font, right-aligned in the 15px gutter column, directly under the disclosure chevron. Selection is a translucent capsule (10px radius, 5px 6px padding) that **hugs its content** — number + thumbnail — rather than spanning the row width, with a 1px inset top-edge highlight; the thumbnail takes **no accent ring** — the capsule alone marks selection, so thumbnails read identically selected or not. Thumbnails are 150px wide with 4px corners and a 1px inset hairline, no drop shadow, no title caption; rows sit 2px apart. Windows and Linux keep the tighter mono-numbered rows (136px thumbnails, 6px gaps) with an accent ring on selection. Numbering is absolute over the whole deck — collapsing a group hides its children but never renumbers the slides after it. Clicking the chevron toggles the group and does not change the selection (the handler stops propagation); clicking anywhere else in the row selects that slide.
