...
- [x] macOS shell scaffolding: `macosApp/Cupboard.xcodeproj` (Gradle framework phase + compose-resources staging), `.app` packaging, IDE run config through the KMP plugin. Terminal launches (run.sh) now activate the app so the window appears without a dock click (`6f8afba`).
...
- [x] Clipboard: cut/copy/paste/duplicate for elements and slides, paste style / copy style. App-internal (presenter-local like undo, not the OS pasteboard); cut+paste keeps position, repeat pastes cascade; collapsed slides travel with their hidden run. (`23ed292`, needs a visual pass)
- [x] Deletion: elements and slides, Clear All; undo/redo re-anchor the slide selection by index when the restored document loses it (the selection-dangle revisit). (`a3a482c`, needs a visual pass)
- [x] Canvas context menu: cut/copy/paste/delete, then z-order, group/ungroup, lock, align/distribute. `ContextClick` through the loop (outside the selection selects, inside keeps it, empty space clears); canvas forwards point + hit only. `NSMenu` on macOS with enablement computed one step ahead of the roundtrip; Compose uses an m3 dropdown at the pointer (`CursorDropdownMenu` is desktop-only API and the chrome is commonMain). Arrange items factored once per shell, shared by menu bar and context menu. WinUI waits for its canvas embed. (`def9f0c`, needs a visual pass)
...
- [ ] On-canvas text editing: caret/selection in text elements, then a real `NSTextInputClient` in `ComposeNSView` for IME (dead keys, CJK).
- [ ] Text formatting: font family/size/weight, bold/italic/underline/strikethrough, color, alignment, line spacing, lists (bullet/numbered, nested), links.
- [ ] Shape catalog: the basic dozen (rect, rounded, oval, triangle, arrow, diamond, star, polygon, quote bubble, callout), lines with arrowheads, fills (solid/gradient/image), borders, shadow, corner radius.
