...
- [ ] Canvas context menu (after clipboard + deletion, so it lands with its top section): cut/copy/paste/delete, then z-order, group/ungroup, lock, align/distribute. Native per shell (`NSMenu`, `CursorDropdownMenu`, `MenuFlyout`) since menus are platform chrome; the canvas only forwards a secondary click with the point + element hit, and enablement reuses the Arrange menus' derivations so the two can't disagree. Right-click on an unselected element selects it first, right-click inside a multi-selection keeps it (new event through the loop, not a canvas-local write). Each shell factors its Arrange items once so the list isn't written twice per platform.
