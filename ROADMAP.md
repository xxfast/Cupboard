...
- [x] Native field context menu on macOS: right-click inside an edit session pops an `NSMenu` (Cut/Copy/Paste, Select All) driving the field through `FieldMenuBridge` + `FieldClipboard`, one code path with the Cmd chords. Compose shells keep the built-in menu. (`324e604`, needs a visual pass)
