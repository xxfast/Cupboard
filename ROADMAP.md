...
- [ ] Spike: `ComposeNSView` hosted in SwiftUI via `NSViewRepresentable`. The riskiest unknown. Compose's macos `Window()` owns its own `NSWindow`; we need to replicate its NSView + `SkiaLayer` internals (see `ComposeWindow.macos.kt` in compose-multiplatform-core) as an embeddable view.
- [ ] Spike: text input on native macOS (`TextField` + IME). The editor is text-heavy; `MacosTextInputService` exists but is unproven.
