...
- ~~Undo/redo~~ done (`91dc202`): bounded document history in the presenter, `Undo`/`Redo` as events, Cmd+Z in both shells' real menus. Disclosure and selection make no history. Revisit when slide/element deletion lands (selection can dangle) and when text editing needs burst coalescing.
