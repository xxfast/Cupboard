...
- [ ] Clipboard while editing: mid-edit, Edit > Cut/Copy/Paste should act on the field's text (Keynote-style) instead of greying out; on macOS a disabled menu key equivalent swallows Cmd+X/C/V before the field sees them, and the native target's in-field clipboard has never been exercised against `NSPasteboard`. Applies to text boxes and code blocks alike; found on the code-editing visual pass.
