...
- [x] Shared screen stores: editor state + mutations live once in `:cupboard` (`EditorStore`, CLAUDE.md constraint 6); shells are thin adapters (Compose reads snapshot state, SwiftUI bridges via subscribe callback + `@Observable`). Navigation stays per platform by design; WinUI 3 will consume the same stores via kotlin-native-nuget. (`01d76a1`) Superseded by the NYTimes-KMP-shaped MVI restructure: `screens/editor/` as `EditorStateModels` / `EditorPresenter` (Molecule) / `EditorViewModel` / `EditorScreen`+`EditorView`, serializable state, subscribe callbacks replaced by a `StateFlow`, autosave absorbed into the presenter, and drags committing once on release instead of per pointer sample. (`9bc97ce`)
...
- Undo/redo: the MVI loop commits one document transition per gesture, which is already undo-entry granularity. Plan: bounded state history in the presenter (skip selection-only events), `Undo`/`Redo` as ordinary events so Cmd+Z works from any shell. Text editing will need burst coalescing. Future.
